...
struct s {
 ...
 int main(){
	...
	FILE *fptr;
	int i;
	fptr=fopen("E:\\Softwares\\C_Programs\\Archit_120_CProgram\\030_ReadWriteFile.txt","wb"); //Opening the file from the location.
	for (i=0;i<1;++i) {
		fflush(stdin);
		printf("Enter Text: ");
	...
	fclose(fptr);
	fptr=fopen("E:\\Softwares\\C_Programs\\Archit_120_CPrograms\\030_ReadWriteFile.txt","rb");
